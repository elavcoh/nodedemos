...
const fs = require('fs'); // ✅ חשוב: נוסיף כדי לבדוק קבצים קיימים
...
// 6. Protect `/app` and serve static files from Client
...
// 7. Static files (JS, CSS, images…)
app.use(express.static(path.join(__dirname, 'Client')));

// ✅ 8. Handle dynamic page routes like /app/favorites
app.get('/app/:page', requireLogin, (req, res, next) => {
  const page = req.params.page;
  const filePath = path.join(__dirname, 'Client', `${page}.html`);
  if (fs.existsSync(filePath)) {
    return res.sendFile(filePath);
  }
  next(); // לא נמצא – המשך הלאה
});

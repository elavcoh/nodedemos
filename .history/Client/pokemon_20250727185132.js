...
// ── Fetch & cache full details (incl. stats) ──────────────
...
async function toggleFavorite(pokemon, card, btn) {
  ...
  // 🌐 Sync with server (ALWAYS, even after removal)
  try {
...
// ── Build a single card with expand & videos ──────────────
function createPokemonCard(pokemon) {
  ...
  // expand/collapse & load videos — THIS handler only affects THIS card
  card.addEventListener("click", async function(e) {
    // if they clicked the button, do not toggle expansion
    if (e.target.closest("button")) return;

    const nowExpanded = this.classList.toggle("expanded");
    if (nowExpanded) {
      const ul = this.querySelector(".yt-list");
      ul.innerHTML = "";
      for (let cat of ["trailer", "gameplay", "anime"]) {
        const vid = await fetchVideoId(pokemon.name, cat);
        const li  = document.createElement("li");
        if (vid) {
          li.innerHTML = `<a href="https://youtu.be/${vid}" target="_blank">
...
async function searchPokemon(query, filter) {
  ...
  try {
    ...
    if (!results.length) {
      ...
      results.forEach(p =>
        pokemonContainer.appendChild(createPokemonCard(p))
      );
    }
  } catch (e) {
    console.error(e);
